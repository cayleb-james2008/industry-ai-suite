...
Import from `suite_core`: `HMACTokenCodec`, `Authenticator`, `Principal`, `AccessPolicy`, `SecurityCore`, `AuditLog`, `FixtureSchema`, `FixtureAdapter`, `FixtureData`, `redact`, `ApprovalAuthority`, `SimulatedSink`, `PromptSentinel`, `GroundedOutputValidator`, `LocalOpenAIClient`, `OpenAICompatibleClient`, `configured_ai_client`, `ai_configuration_status`, `NonAIFallback`, and their documented error/result types.
...
Only call `issue(... confirmed=True)` after both the trusted security core accepts the signed approver token for `approve` and the UI/service boundary receives explicit human confirmation. The trusted identity service—not the requesting worker—must issue the signed approver token after authenticating the human. Never let the requesting worker approve its own action. The public `verify` method never accepts a caller-selected current time; deterministic clocks are injected only when constructing an authority in tests. A missing, expired, forged, wrong-role, cross-tenant, wrong-evidence, self-approval, or wrong-action token is refused before the sink records anything, and denial is audited without evidence contents.

## Audit and personal-data handling

`AuditLog` is an append-only JSONL writer with fixed fields: UTC time, actor, tenant, role, action, allowlisted evidence IDs, approval boolean, approving actor, and outcome. It does not accept arbitrary payloads or credentials. Write failures raise; the protected operation must stop. Call `redact(value)` before ordinary outputs/logging: sensitive nested and compound keys, credential-bearing headers, URL queries/fragments and URL user-info, recognizable email/phone/account/street patterns, common service tokens, person-bearing field values, and capitalized name-like word pairs are redacted. `project_source_metadata` preserves declared identifier and source-label fields while redacting personal values elsewhere. The name detector uses an explicit organization/provenance allowlist and remains incomplete; its allow/deny model and limitations are documented in [`SECURITY.md`](SECURITY.md). This is minimization, not a substitute for app-specific data classification; apps should avoid collecting unnecessary personal data in the first place. Use opaque identifiers for actors, tenants, actions, and evidence.
...
`LocalOpenAIClient` defaults to `http://127.0.0.1:52652/v1`, accepts only a literal loopback IP over plain HTTP, disables proxy use, probes `/models`, and then can make OpenAI-compatible `/chat/completions` calls. It never contacts hosted endpoints or handles API credentials. `OpenAICompatibleClient` supports the local loopback route without credentials; non-loopback routes are refused before network access by default. An external user may explicitly opt into their own hosted route only with `SUITE_AI_ALLOW_HOSTED=true`, an exact hostname in `SUITE_AI_ALLOWED_HOSTS`, and an exact selected model in `SUITE_AI_ALLOWED_MODELS`; this is a paid-capable bring-your-own integration, not a free route. The opt-in is off in this run and no hosted calls were made. Configured routes use safe runtime credential loading, refuse redirects, write redacted append-only **app-reported** traces, and capture provider-returned identity/usage. The separate standard-library witness proxy holds a fresh HMAC-SHA256 key only in memory, appends keyed body-free transport metadata with a previous-line hash chain, then reveals the key and final chain head on its own stdout only after stopping. `scripts/verify_witness.py` separately verifies that chain and matches app-reported call IDs and exact request/response hashes. It is not imported by apps, `suite_core`, or `scripts/run_all.py`; neither apps nor the runner labels any call `VERIFIED`. Public endpoints require HTTPS and a credential; plain HTTP is permitted only for a literal loopback IP and explicit port. Both clients cap prompt length, response size, output tokens, and request time. The single configuration reference, including the environment variables and safe provider examples, is [`SECURITY.md`](SECURITY.md).
...
`NonAIFallback().result(reason=..., evidence_ids=...)` returns `NON-AI / DETERMINISTIC FALLBACK` and a manual evidence handoff. It does not simulate or generate an AI answer. Without a configured route, app output says `AI: unavailable (UNVERIFIED)` and the non-AI workflow continues. `python3 -m suite_core doctor` reports whether configuration is present without printing secrets; it does not probe the network. The configured runner flag is `python3 scripts/run_all.py --ai-configured`.

**AI proof limitation:** provider-returned identifiers and a separate local witness log make a specific exchange inspectable, but do not provide cryptographic provider attestation. The app's own receipt and trace always remain explicitly app-reported, and the runner always leaves calls at `AI CANDIDATE (unwitnessed)`. Only the separate verifier can return `VERIFIED` for a complete HMAC-and-chain-valid transport match; that does not establish task quality, grounding, or overall workflow completeness. Keep the suite **INCOMPLETE** without the remaining lead evidence.
...
- The checked-in demo is deliberately deterministic and **not AI**. A real completion, exact model identity/version, and AI quality remain unverified until a lead-owned independent check.
